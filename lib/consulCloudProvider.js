 ...
 * @param {String}  [providerOptions.caBundle]        - Absolute TMSH path to a bundle of one or
 *                                                      more trusted CA certificates in PEM format
 *                                                      that overrides the default Nodejs
 *                                                      certificates.
 *
 *     "/Common/myCert.pem"
 *
    ...
    if (this.providerOptions.caBundle) {
        ...
        promises.push(
            getCertFilePath(this.providerOptions.caBundle)
                .catch((err) => {
                    return q.reject(new Error(`caBundle: ${err.message}`));
                })
        );
    }
...
function getCertFilePath(tmshPath) {
    if (typeof tmshPath !== 'string') {
        return q.reject(new Error('TMSH path must be a string'));
    }
    ...
    const FILE_PREFIX = splitPath.join(':');
    const CERT_DIR = `/config/filestore/files_d/${FOLDER}_d/certificate_d/`;

    return cloudUtil.runShellCommand(`ls -1t ${CERT_DIR}`)
        .then((response) => {
            const files = response.split('\n');
            const certName = files.find((element) => {
                return element.startsWith(FILE_PREFIX);
            });

            if (!certName) {
                return q.reject(new Error('Certificate does not exist'));
            }
            return q(CERT_DIR + certName);
        });
}
